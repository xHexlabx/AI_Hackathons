"""HexTex Kaggriculture agent v7 - dairy & berries economy with reserve-price selling.
...
PARAMS = {
    ...
    "shed_pressure": 80,
    "fert_use_below": 30,  # use fertiliser on wheat/carrot only when it is worth less than this
    # labour
    "max_hands": 12,
    "actions_per_unit": 20,
    "walk_factor": 1.3,
    ...
    def market_orders(self, obs, me, private, day, hour, carried, stats, dropping):
        ...
        if not final_day:
            for x, y in owned:
                ...
                if (
                    t["crop"] == "STRAWBERRY"
                    and age <= max(self.p["straw_fert_ages"])
                    and age >= min(self.p["straw_fert_ages"]) - 2
                ) or (
        ...
        if hour >= 1 and n_quads < 4 and day <= self.p["last_land_day"]:
            ...
            if money - cost >= reserve + margin and (short_of_land or n_quads < 3):
        ...
        if hour >= 1 or day == 0:
            ...
            targets = {
                "COW": self.p["cow_cap"] if day <= self.p["last_cow_day"] else 0,
                "SHEEP": min(self.p["sheep_cap"], yarn * self.p["sheep_per_yarn"])
                if day <= self.p["last_sheep_day"]
                else 0,
                "GOOSE": self.p["geese_cap"] if day <= 18 else 0,
            }
            if day == 0:
                targets["COW"] = min(targets["COW"], self.p["open_cows"])
                targets["SHEEP"] = (
                    max(targets["SHEEP"], self.p["open_sheep"])
                ...
                if cd["ongoing"]:
                    if t["yield_units"] > 0:
                        add(
                            pos,
                            ["HARVEST"],
                            74 if (t["yield_units"] >= 2 or final_day or hour >= 18) else 52,
                        )
                    if final_day:
                        continue
                    ...
                    if age in straw_ages and t["fertilized_until_day"] < day:
                        fert_targets += 1
...
def agent(obs, config=None):
    ...
    try:
        ...
        print(f"[hextex_v7] step {obs.get('step')} error: {exc!r}")
