"""HexTex Kaggriculture agent v6 - goose economy, zone-based feeding logistics, labour-capped crops.
...
class Brain:
    ...
    def plan_day(self, obs, me, day):
        ...
        cafes = sum(1 for s in (obs.get("town") or {}).get("unlocked_shops", []) if s == "PET_CAFE")
    ...
    def crop_budget(self, me, day):
        """Only plant as many crop tiles as the labour left after the animals can water."""
        tiles = me["tiles"]
        ...
        budget = n_units * self.p["actions_per_unit"] - n_animals * self.p["goose_actions"]
        cap = max(self.p["min_crop_tiles"], int(budget / self.p["crop_actions"]))
        cands = []
        for x, y in self.owned(tiles):
            t = tiles[y][x]
            r = self.role((x, y))
            if r in ANIMALS or is_animal(t) or is_empty_structure(t):
                continue
            if is_plant(t) or (r in CROPS and self.can_plant(r, day)):
                # keep existing plants first, then the nearest free tiles
                cands.append((0 if is_plant(t) else 1, shed_dist((x, y)), (x, y)))
        cands.sort()
        return {pos for _, _, pos in cands[:cap]}
    ...
    def make_zones(self, me, day):
        ...
        for x, y in self.owned(tiles):
            ...
            elif r in CROPS and self.can_plant(r, day) and (x, y) in self.crop_allowed:
                w = 3.0
            else:
                continue
            if is_plant(t) and (x, y) not in self.crop_allowed:
                continue
    ...
    def market_orders(self, obs, me, private, day, hour, carried, stats, dropping):
        ...
        if hour >= 20 and day < LAST_DAY - 1:
            ...
            need = max(need, min(unfed, 4))  # wheat is stuck in the wrong hands - restock the shed
    ...
    def unit_actions(self, obs, me, private, day, hour, units, invs, carried, stats):
        ...
        zones = self.zones
        # zone-aware wheat pickup: each unit fetches the ration of the unfed animals in its zone
        wheat_need: dict[int, int] = {}
        unassigned = 0
        for x, y in owned:
            t = tiles[y][x]
        ...
        for i, pos in enumerate(units):
            ...
            for tk in tasks:
                if tk["key"] in claimed:
                    continue
                if tk.get("unit") is not None and tk["unit"] != i:
                    continue
                ...
                score = (
                    ...
                    + (zone_b if zones.get(tk["pos"]) == i else 0)
                )
                if score > best_score:
...
def agent(obs, config=None):
    ...
    try:
        ...
        print(f"[hextex_v6] step {obs.get('step')} error: {exc!r}")
