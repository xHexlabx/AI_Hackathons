"""HexTex Kaggriculture agent v22 - v21 with stable day routes (hand i -> cluster i), full wheat pickups, late-day feed safety.
...
PARAMS = {
    ...
    "open_wheat": 7,
    "open_hands": 5,
    "open_feed_wheat": 4,  # day-1 ration for the opening herd (bought with the leftover cash)
    ...
    "cow_cap": 13,
    "sheep_cap": 22,
    # clone-line baseline herd (day -> target), a floor under the demand-aware targets
    "cow_sched": {"0": 2, "3": 3, "4": 4, "7": 6, "8": 8},
    "sheep_sched": {"0": 2, "9": 4, "11": 6},
    "geese_sched": {"12": 3},
    "feed_safety": True,  # never buy an animal that cannot be fed today and tomorrow
    "last_cow_day": 14,
    "last_sheep_day": 16,
    "geese_cap": 10,
    "eggs_per_goose": 2.0,
    ...
    "cash_floor": 20,
    "feed_days_reserve": 0.4,  # the clone line runs at ~$0 cash until the melon dump
    "wheat_stock_days": 2.0,  # a full day of feed must be in the shed at hour 0 for the pickups
    ...
    "melon_wave2_tiles": 0,  # second melon wave off: ~$2k for 13 seeds and ~150 actions (A/B: +0.8k)
    "wheat_tiles_cap": 12,
    # labour
    "max_hands": 12,
    "actions_per_unit": 20,
    ...
    "hire_cash_frac": 0.3,
    "animal_actions": 5.0,
    "crops_far": False,  # (tested: near tiles for crops scored better over 24 games)
    # step 5/6 toggles (clone line): wheat tiles by day, land days, carrot finisher
    "wheat_cap_sched": None,  # e.g. {"0": 7, "12": 25, "22": 40}; None = wheat_tiles_cap
    "land_mode": "v20",  # "clone" = NE on day 6, SW on day 11, never SE
    "land_days": {"2": 6, "3": 11},
    "carrot_finisher_days": [24, 26],
    "carrot_finisher_tiles": 0,  # 0 = off; clone plants ~30 carrots on d25-27
    "wheat_fert_age": [
        2,
        3,
    ],  # clone top seats fertilise wheat at age 1-2  # feed + care + harvest + collect + the step onto the tile
    ...
    "route_pickup_max": 12,
    "feed_safety_hour": 16,  # from this hour a unit carrying wheat feeds any unfed animal it can reach
    "help_radius": 4,  # idle units only help within this distance (walking further is a net loss)
    "evening_return": False,  # the engine drops inventories into the shed at end of day for free
    "wheat_stock_margin": 2,
...
# ---------------------------------------------------------------------------- helpers
def sched_value(sched, day, default=0):
    keys = [int(k) for k in sched if int(k) <= day]
    return sched[str(max(keys))] if keys else default
...
class Brain:
    ...
    # ------------------------------------------------------------------ helpers
    def apply_param_overrides(self):
        CROPS["WHEAT"]["fert_age"] = tuple(self.p["wheat_fert_age"])
    ...
    def animal_targets(self, obs, day):
        ...
        cows = min(self.p["cow_cap"], max(cows, sched_value(self.p["cow_sched"], day, 0)))
        sheep = min(self.p["sheep_cap"], max(sheep, sched_value(self.p["sheep_sched"], day, 0)))
        geese = min(self.p["geese_cap"], max(geese, sched_value(self.p["geese_sched"], day, 0)))
        if day == 0:
            cows, sheep, geese = self.p["open_cows"], self.p["open_sheep"], 0
        return {
    ...
    def plan_day(self, obs, me, day):
        ...
        if self.can_plant("STRAWBERRY", day):
            ...
            # pace the planting like the clone until day 11 (cash goes to the herd first),
            # then the second wave on the freed melon tiles: at least the clone's 33
            sched_want = sched_value(self.p["straw_sched"], day, 0)
            want = (
                min(want, sched_want)
                if day < 11
                else min(self.p["straw_cap"], max(want, sched_want))
            )
            ...
            if want > have:
                ...
                cands.sort(key=(lambda p: -shed_dist(p)) if self.p["crops_far"] else shed_dist)
        ...
        lo_c, hi_c = self.p["carrot_finisher_days"]
        if self.p["carrot_finisher_tiles"] and lo_c <= day <= hi_c:
            want_c = max(want_c, self.p["carrot_finisher_tiles"])
    ...
    def crop_budget(self, me, day):
        ...
        for _, _, pos, w in cands:
            ...
            if crop == "WHEAT":
                wheat_cap = (
                    sched_value(self.p["wheat_cap_sched"], day, self.p["wheat_tiles_cap"])
                    if self.p["wheat_cap_sched"]
                    else self.p["wheat_tiles_cap"]
                )
                if wheat_n >= max(wheat_cap, n_animals) and not is_plant(t):
                    continue
                wheat_n += 1
    ...
    def market_orders(self, obs, me, private, day, hour, carried, stats, dropping):
        ...
        herd = n_animals + sum(stats[a]["stock"] for a in ANIMALS)
        wheat_keep = 0 if final_day else math.ceil(herd * self.p["wheat_stock_days"])
        fert_keep = 0
        if not final_day:
            for x, y in owned:
                t = tiles[y][x]
        ...
        money = me["money"]
        if day == 0 and hour == 0:
            # clone-line opening in cash-safe order: cheap hires, seeds, animals, then a day-1 ration
            opening = [["HIRE"] for _ in range(min(self.p["open_hands"], 5))]
            opening.append(["BUY_SEED", "MELON", self.p["open_melons"]])
            opening.append(["BUY_SEED", "WHEAT", self.p["open_wheat"]])
            if self.p["open_cows"]:
                opening.append(["BUY_ANIMAL", "COW", self.p["open_cows"]])
            if self.p["open_sheep"]:
                opening.append(["BUY_ANIMAL", "SHEEP", self.p["open_sheep"]])
            if self.p.get("open_feed_wheat", 0):
                opening.append(["BUY_PRODUCT", "WHEAT", self.p["open_feed_wheat"]])
            return (orders + opening)[:MAX_ORDERS]
        # 2) feed
        ...
        if hour >= 20 and day < LAST_DAY - 1:
            need = max(need, n_animals + self.p["wheat_stock_margin"] - wheat_avail)
        ...
        if self.p["land_mode"] == "clone":
            if hour >= 1 and n_quads < self.p["max_quadrants"] and day <= self.p["last_land_day"]:
                need_day = int(self.p["land_days"].get(str(n_quads + 1), 99))
                cost = LAND_PRICES[n_quads - 1]
                if day >= need_day and money - cost >= reserve:
                    orders.append(["BUY_LAND"])
                    money -= cost
        elif hour >= 1 and n_quads < 4 and day <= self.p["last_land_day"]:
            cost = LAND_PRICES[n_quads - 1]
        ...
        if hour >= 1 or day == 0:
            ...
            for a in ("COW", "SHEEP", "GOOSE"):
                ...
                if self.p["feed_safety"] and day > 0 and want > 0:
                    while (
                        want > 0
                        and money - want * ANIMALS[a]["cost"] < (n_animals + want) * wheat_price
                    ):
                        want -= 1  # keep one day of feed money after the purchase
                if want <= 0:
                    continue
        ...
        # 5) seeds (while the herd is below schedule, seeds get only their share of the cash)
        seed_money = money
        if day > 0 and gap > 0:
            share = self.p["rush_seed_share"] if rush else self.p["seed_cash_share"]
            seed_money = min(money, reserve + share * max(0.0, me["money"] - reserve))
        for crop in ("MELON", "STRAWBERRY", "TOMATO", "CARROT", "WHEAT"):
            ...
            if short > 0:
                cap_money = (
                    money if crop == "WHEAT" else seed_money
                )  # wheat is feed: always allowed
                spend_cap = cap_money - (reserve if day > 0 else self.p["cash_floor"])
                qty = min(short, int(max(0.0, spend_cap) // CROPS[crop]["seed"]))
                if qty > 0:
                    orders.append(["BUY_SEED", crop, qty])
                    money -= qty * CROPS[crop]["seed"]
                    seed_money -= qty * CROPS[crop]["seed"]
        # 6) hires
        to_hire = self.hires_wanted - me["hires_today"]
        for _ in range(max(0, min(to_hire, MAX_ORDERS - len(orders)))):
            orders.append(["HIRE"])
    ...
    def unit_actions(self, obs, me, private, day, hour, units, invs, carried, stats):
        ...
        for x, y in owned:
            ...
            if is_free(t):
                ...
                a = ANIMALS[t["animal"]]
                wheat_somewhere = shed.get("WHEAT", 0) + carried.get("WHEAT", 0) > 0
                # an animal placed today survives tonight unfed (consecutive_unfed starts at 0):
                # save the ration for tomorrow when cash is still thin
                needs_feed = not t["fed_today"] and not (
                    t.get("placed_day", -1) == day
                    and t.get("consecutive_unfed", 0) == 0
                    and day <= 2
                )
                gate = (
                    "WHEAT"
                    if (not final_day and needs_feed and hour < 18 and wheat_somewhere)
                    else None
                )
                if not final_day:
                    if needs_feed:
        ...
        if route_mode:
            ...
            crew = max(len(units), 1 + self.hires_wanted)
            key = (day, crew)
            if self.routes_key != key or not self.routes:
                # build the day's clusters once for the planned crew; hands keep their index all
                # day (the hands list only appends), so cluster i belongs to unit i for the whole day
                # and the wheat/fertiliser a unit picked up at spawn stays with the right tiles
                clusters = self.make_routes(tile_tasks, crew)
                self.routes = {i: clusters.get(i, []) for i in range(crew)}
                self.routes_key = key
                self.route_target = {}
        ...
        if route_mode:
            ...
            for i, pos in enumerate(units):
                ...
                scan = list(route)
                if hour >= self.p["feed_safety_hour"] and inv.get("WHEAT", 0) > 0:
                    # late in the day a wheat carrier may rescue unfed animals outside its route
                    scan += [
                        rp
                        for rp, tks in tile_tasks.items()
                        if rp not in route and any(tk["op"][0] == "FEED" for tk in tks)
                    ]
                for rp in scan:
                    if rp == pos:
                        continue
                    ...
                    if rp not in route:
                        tks = [tk for tk in tks if tk["op"][0] == "FEED"]
                    if tks:
                        cands.append(
        ...
        for i, pos in enumerate(units):
            ...
            for tk in tasks:
                ...
                if (
                    ...
                    if hour < self.p["help_hour"] and tk["prio"] < 100:
                        continue
                    # leave feeding to the route owner (who carries the wheat) until late in the day
                    if tk["op"][0] == "FEED" and hour < self.p["feed_safety_hour"]:
                        continue
                    if (
                        dist(pos, tk["pos"]) > self.p["help_radius"]
                        and tk["prio"] < 100
                        and hour < self.p["emergency_hour"]
                    ):
                        continue
            ...
            if load > 0:
                ...
                elif hour + sd >= 21 and (self.p["evening_return"] or sd <= 1):
                    dprio = 120
                elif premium_load >= self.p["route_premium_drop"] or (premium_load > 0 and sd <= 1):
                    dprio = 84
...
def agent(obs, config=None):
    ...
    try:
        ...
        print(f"[hextex_v22] step {obs.get('step')} error: {exc!r}")
