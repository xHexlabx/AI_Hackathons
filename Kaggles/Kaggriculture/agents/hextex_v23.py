"""HexTex Kaggriculture agent v23 - anti-meta: geese-first opening (fertiliser + egg pots are uncontested by the
ladder clone line), lighter cow/sheep floors, melon dump before the clone on day 10.
...
class Brain:
    ...
    def plan_day(self, obs, me, day):
        ...
        if day == 0 and not self.roles:
            ...
            for pos in by_shed[: self.p.get("open_geese", 0)]:
                self.roles[pos] = "GOOSE"
            by_shed = [p for p in by_shed if p not in self.roles]
    ...
    def market_orders(self, obs, me, private, day, hour, carried, stats, dropping):
        ...
        if day == 0 and hour == 0:
            ...
            if self.p.get("open_geese", 0):
                opening.append(["BUY_ANIMAL", "GOOSE", self.p["open_geese"]])
            if self.p["open_cows"]:
    ...
    def unit_actions(self, obs, me, private, day, hour, units, invs, carried, stats):
        ...
        melon_ready = any(
            is_plant(tiles[y][x])
            and tiles[y][x]["crop"] == "MELON"
            and day - tiles[y][x]["planted_day"] >= 10
            for x, y in owned
        )
        melon_rush = melon_ready and hour < self.p["melon_rush_hours"]
        route_mode = self.p["route_mode"] and not final_day and not melon_rush
        tile_tasks = {}
        if route_mode:
            for tk in tasks:
...
def agent(obs, config=None):
    ...
    try:
        ...
        print(f"[hextex_v23] step {obs.get('step')} error: {exc!r}")
